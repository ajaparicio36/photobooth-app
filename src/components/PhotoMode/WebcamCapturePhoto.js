...
const WebcamCapturePhoto = ({ setPhotos, setCurrentPage, paperType, }) => {
    ...
    if (!webcamReady) {
        return (_jsxs("div", { className: "flex flex-col items-center justify-center h-screen bg-gray-100", children: [_jsx("div", { className: "text-xl mb-4", children: "Initializing webcam..." }), _jsx("div", { className: "text-sm text-gray-600 mb-4", children: "Please allow camera access if prompted" }), _jsx("div", { className: "text-xs text-gray-500 mb-4", children: "Check browser console for detailed logs" }), _jsx("button", { onClick: () => {
                        console.log("Manual retry clicked");
                        setWebcamReady(false);
                        initializeWebcam();
                    }, className: "bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded", children: "Retry Webcam" })] }));
    }
    // Check if we have a valid stream
    const hasValidStream = streamRef.current && streamRef.current.getTracks().length > 0;
    if (!hasValidStream) {
        return (_jsxs("div", { className: "flex flex-col items-center justify-center h-screen bg-gray-100", children: [_jsx("h1", { className: "text-2xl font-bold mb-4", children: "Webcam Not Available" }), _jsx("p", { className: "mb-4 text-center", children: "Failed to access webcam. Please check your camera permissions." }), _jsx("button", { onClick: () => {
                        setWebcamReady(false);
                        initializeWebcam();
